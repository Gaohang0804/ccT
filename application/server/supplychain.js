...
supplychainRouter.route('/orders/:id').get(function (request, response) {
    submitTx(request, 'queryOrder', request.params.id)
        .then((queryOrderResponse) => {
            ...
            console.log(`order ${order.orderId} : price = ${order.price}, quantity = ${order.quantity}, producer = ${order.producerId}, consumer = ${order.retailerId}, trackingInfo = ${order.trackingInfo}, adress = ${order.adress}, ship to adress = ${order.adress1}, state = ${order.currentOrderState}`);
            response.status(STATUS_SUCCESS);
            response.send(order);
        }, (error) => {
            response.status(STATUS_SERVER_ERROR);
